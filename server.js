...
function mergePropertiesText(rawOriginal, updates) {
    ...
    if (newKeys.length) {
        ...
        for (const k of newKeys) out.push(`${k}=${escape(updates[k])}`);
    }
    return out.join('\n');
}
...
app.get('/api/server-properties', (req, res) => {
    try {
        ...
        if (fs.existsSync(propsPath)) {
            ...
            raw = DEFAULT_SERVER_PROPERTIES;
            generated = true;
        }
        res.json({ properties: parseProperties(raw), raw, generated });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/server-properties', (req, res) => {
    try {
        ...
        if (typeof raw === 'string') {
            ...
            return res.json({ success: true, mode: 'raw' });
        }
        if (properties && typeof properties === 'object') {
            let original = '';
            if (fs.existsSync(propsPath)) original = fs.readFileSync(propsPath, 'utf8');
            else original = DEFAULT_SERVER_PROPERTIES;
            const next = mergePropertiesText(original, properties);
            fs.writeFileSync(propsPath, next, 'utf8');
            return res.json({ success: true, mode: 'merge' });
        }
        res.status(400).json({ error: 'Missing properties or raw body' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
